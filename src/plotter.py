...
class StockPlotter:
    ...
    def save_batch_report(
        ...
        # 构建快速切换按钮HTML（显示股票名称）
        buttons_html = ''.join(f'<span class="quick-btn active" onclick="quickSwitch(\'{c}\')">{stock_data[c]["name"]}</span>' for c in all_codes[:8])
...
''')
        ...
        function applyFilter() {
            ...
            if (filteredCodes.length === 0) {
                ...
                selector.innerHTML = filteredCodes.slice(0, 8).map(c =>
                    '<span class="quick-btn' + (c === currentStock ? ' active' : '') + '" onclick="quickSwitch(\\'' + c + '\\')">' + (stockData[c] && stockData[c].name ? stockData[c].name : c) + '</span>'
                ).join('');
            }
        ...
        function showCharts() {
            currentView = 'charts';
            document.getElementById('chartsView').style.display = 'flex';
